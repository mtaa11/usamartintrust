<template> 
	<view class="boardLog">
		<view class="font-24 font-B color-7 font-c margin-t22 margin-b28">每日收益明细</view>
		<view class="dataList">
			<view class="List">
				...
				<view class="ListItem" v-for="(item,i) of list">
					<view class="db-flex db-space-between margin-b3">
						...
						<view class="font-14 font-B color-11">{{item.num}}MTU</view>
					</view>
	...
	export default {
		...
		methods: {
			showdata() {
				...
				this.$request("/index/liu/everydayprofit", "POST",{pid:pid}).then(res => {
					if (res.code == 0) {
						this.list=res.list
					}
				})
